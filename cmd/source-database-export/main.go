...
func main() {
	var input = flag.String("input", "", "path to exported database data file (CSV/JSON)")
	flag.Parse()
	...
	sink := func(ctx context.Context, res examplesupport.IngestionResult) error {
		examplesupport.PrintOutcome(os.Stdout, "source-database-export", res.Outcome, fmt.Sprintf("rows=%d input=%s", res.Records, absInput))
		return nil
	}

	fmt.Fprintln(os.Stdout, "[source-database-export] trigger fired")
	fmt.Fprintln(os.Stdout, "[source-database-export] ingestion started")
	_, err = pipeline.New("source-database-export", src).
		Then(process.Handle).
		To(sink).
		Run(context.Background())
	if err != nil {
		fmt.Fprintf(os.Stderr, "[source-database-export] error: %v\n", err)
		os.Exit(1)
	}
	fmt.Fprintln(os.Stdout, "[source-database-export] ingestion finished")
}
